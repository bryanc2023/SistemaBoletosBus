...
public class BoletoController {
    ...
    @GetMapping("/boleto/{id}/estaDisponible")
    public String verificarDisponibilidad(@PathVariable(value = "id") Integer id, RedirectAttributes flash) {
        if (id > 0) {
            flash.addFlashAttribute("success",
                    boletoService.findById(id).isStockBoletos()
            );
        }
        return "redirect:/boleto/listar";
    }
